...
// export class Geocoder {
//     options: any;
//     // URL для обычного (адресного) геокодирования – используется, если передана строка
//     api: string;
//     // URL для реверс‑геокодирования (по координатам) – используется, если передан массив координат
//     apiReverse: string;
//     private http: HttpClient;
//     private selectedResult: any = null;
//     private _results: any[] = [];
...
//       // Если запрос реверс (options.isReverse === true) – используем другой URL
//       // В нашем случае: если передана строка (isReverse не выставлен или false), используем getDetails
//       // Если же options.isReverse === true (т.е. переданы координаты), используем другой эндпоинт (например, v1/geocode)
//       this.api = this.options.isReverse ? `${environment.apiUri}v1/geocode` : `${environment.apiUri}v1/getDetails`;
//       this.http = http;
//     }
...
//     /**
//      * Выполняет запрос геокодирования или реверс‑геокодирования.
//      * @param query — строка запроса (если обычное геокодирование) или строка координат (например, "lng,lat") для реверс‑геокодирования.
//      */
//     private geocode(query: string, isReverse: boolean): Observable<any[]> {
//       this.loading$.next();
//       const paramsObj: any = {};
//       // В зависимости от типа запроса формируем параметры.
//       if (isReverse) {
//         // Если запрос по координатам, например, передаём их как latlng или другой параметр, согласно спецификации вашего API
//         paramsObj.latlng = query;
//       } else {
//         // Если строка – адрес, передаём его как placeId (или другой параметр)
//         paramsObj.placeId = query;
...
//       // Исключаем поля, которые не нужны в запросе
//       const exclude = ['placeholder', 'zoom', 'flyTo', 'accessToken', 'api', 'apiReverse', 'isReverse'];
//       Object.keys(this.options || {}).forEach(key => {
//         if (exclude.indexOf(key) === -1) {
//           paramsObj[key] = this.options[key];
...
//       const params = new HttpParams({ fromObject: paramsObj });
//       // Выбираем URL в зависимости от типа запроса: если isReverse true – используется api (уже установлен в конструкторе)
//       const url = this.api;
...
//         }),
//         finalize(() => {
//           // Здесь можно завершить loading, если требуется
//         })
//       );
//     }
...
//         q = query;
//       }
//       this.geocode(q, isReverse).subscribe({
//         next: data => {
//           if (data) {
//             this.selectedResult = data;
//             this.result$.next(this.selectedResult);
