...
export class MapSidebarComponent implements OnDestroy, OnInit {
    ...
    public state: any[] = [];
    public style: string;
    public mapLanguage = { name: 'Английский', code: 'en' };
    public mapBaseLayer = { name: 'Улицы', code: 'streets-v12' };
    private _map: any;
    private _destroy$ = new Subject<boolean>();

    public defaultOpts: any = {
        language: { name: 'Английский', code: 'en' },
        baseLayer: { name: 'Улицы', code: 'streets-v12' },
        customLayers: null
    }

    public languageOptions: any[] = [
        { name: 'Английский', code: 'en' },
        { name: 'Китайский', code: 'zh-Hans' },
        { name: 'Французкий', code: 'fr' },
        { name: 'Немецкий', code: 'de' },
        { name: 'Испанскй', code: 'es' },
        { name: 'Русский', code: 'ru' },
    ];

    public baseLayersOptions: any[] = [
        { name: 'Cпутник', code: 'satellite-streets-v12' },
        { name: 'Светлый', code: 'light-v11' },
        { name: 'Темный', code: 'dark-v11' },
        { name: 'По умолчанию', code: 'streets-v12' },
        { name: 'Улицы', code: 'outdoors-v12' },
        { name: 'Навигация (светлая)', code: 'navigation-day-v1' },
        { name: 'Навигация (темная)', code: 'navigation-night-v1' }
    ];

    public customLayersOptions: any[] = [
        { name: 'Границы стран', code: 'countryBorders' },
        { name: 'Пункты пропуска', code: 'checkPoints' },
        { name: 'АЗС', code: 'gasStation' },
        { name: 'СТО', code: 'inspectionPoints' }
    ]
    ...
    private _watchForLoadChanges() {
        ...
        //         const selectedLanguage = (language && JSON.parse(language)) ? JSON.parse(language) : { name: 'Английский', code: 'en' };
    ...
    public ngOnInit(): void {
        ...
        this.form.get('customLayers')?.valueChanges
            .pipe(delay(0))
            .subscribe((data: any[]) => {
                localStorage.setItem('customLayers', JSON.stringify(data));
            });
    }
    ...
    private _buildForm(): void {
        ...
        const selectedBaseLayers = (baseLayers && JSON.parse(baseLayers)) ? JSON.parse(baseLayers) : { name: 'По умолчанию', code: 'streets-v12' };
